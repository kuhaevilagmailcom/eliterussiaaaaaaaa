...
def build_router(
    ...
    async def send_screen(
        ...
        *,
        ...
        # Ordinary sections reuse the same bot message instead of creating
        # a new one on every click.
        if last_id and not bottom_menu:
            try:
        ...
        # The persistent bottom ReplyKeyboard cannot be attached by editing.
        # When returning home, delete the previous screen first, then create
        # exactly one replacement. No duplicate menu messages accumulate.
        if bottom_menu and last_id:
